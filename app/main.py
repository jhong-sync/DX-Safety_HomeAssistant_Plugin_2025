...
async def main():
    ...
    log.info(f"원격 MQTT 인게스터 생성 완료 host:{s.remote_mqtt.host} port:{s.remote_mqtt.port} topic:{s.remote_mqtt.topic} username:{s.remote_mqtt.username} password:{s.remote_mqtt.password} tls:{s.remote_mqtt.tls} client_id:{s.remote_mqtt.client_id}")
    
    outbox = SQLiteOutbox(s.reliability.outbox_path); await outbox.init()
    log.info(f"Outbox 저장소 생성 완료 path:{s.reliability.outbox_path}")
    ...
    log.info(f"로컬 MQTT 퍼블리셔 생성 완료 host:{s.local_mqtt.host} port:{s.local_mqtt.port} topic_prefix:{s.local_mqtt.topic_prefix} username:{s.local_mqtt.username} password:{s.local_mqtt.password} tls:{s.local_mqtt.tls} client_id:{s.local_mqtt.client_id}")
    
    idem = SQLiteIdemStore(s.reliability.idem_path, s.reliability.idempotency_ttl_sec); await idem.init()
    log.info(f"Idempotency 저장소 생성 완료 path:{s.reliability.idem_path} ttl_sec:{s.reliability.idempotency_ttl_sec}")
    ...
    try:
        async with ha:
            ...
            if not coords:
                ...
                log.info(f"HA 홈 좌표 조회 성공 lat:{coords[0]} lon:{coords[1]}")
